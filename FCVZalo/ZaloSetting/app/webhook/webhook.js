        ...
        $.ajax(setting).done(function (rs) {
            ...
            if (rs != undefined && rs.Data != undefined) {
                ...
                for (var i = 0; i < rs.Data.length; i++) {
                    ...
                    html += `<td>${data.EventName != null ? data.EventName : ''}</td>`;
                    html += `<td style="word-break: break-all;">${data.ResponseContent != null ? data.ResponseContent : ''}</td>`;
                    html += `<td class="text-center">${data.eventtime != null ? format_date_dd_mm_yyyy_hh_mm_ss(data.eventtime) : ''}</td>`;
