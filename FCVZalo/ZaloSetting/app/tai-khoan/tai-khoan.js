        ...
        $.ajax(setting).done(function (rs) {
            ...
            if (rs != undefined && rs.Data != undefined) {
                ...
                for (var i = 0; i < rs.Data.length; i++) {
                    ...
                    html += `<td class="text-center"><img src="${data.Avatar != null ? data.Avatar : ''}" width="50" /></td>`;
                    html += `<td><input class="form-control txtID" disabled value="${data.ID != null ? data.ID : ''}" spellcheck="false" autocomplete="off" /></td>`;
                    html += `<td><input class="form-control txtDisplayName" disabled value="${data.DisplayName != null ? data.DisplayName : ''}" spellcheck="false" autocomplete="off" /></td>`;
                    html += `<td><textarea class="form-control" disabled rows="3">${data.Description != null ? data.Description : ''}</textarea></td>`;
                    html += `<td class="text-center"><input disabled type="checkbox" ${data.IsVerified == true ? 'checked' : ''} /></td>`;
